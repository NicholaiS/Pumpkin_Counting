...
def main():
    ...
    with rasterio.open('Input files/PumpkinField.tif') as src:
        ...
        with open(position_file, "w") as log_file:
            for chunk_num, (chunk_data, window, x, y) in enumerate(get_chunks(bgr, chunk_size)):
                ...
                chunk_path = os.path.join(chunk_folder, f"Chunk_{chunk_num}.png")
                ...
                chunk_data, count, pumpkin_positions_chunk = extract_features(img_maha, chunk_data, chunk_num)
                ...
                for adjusted_pos in adjusted_pumpkin_positions:
                    ...
                    for pos in pumpkin_positions.keys():
                        ...
                        if pos[0] - 5 <= adjusted_pos[0] <= pos[0] + 5 and pos[1] - 5 <= adjusted_pos[1] <= pos[1] + 5:
                            is_duplicate = True
                    ...
                    if not is_duplicate:
                        pumpkin_positions.setdefault(adjusted_pos, True)
                        
    stitch_chunks(input_folder, position_file, output_folder, chunk_size)
    field = load_image("Outputs/stitched_image.png")
    ...
    save_image(field, "Outputs/stitched_image_marked.png")
